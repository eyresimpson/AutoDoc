...
const registerFormatter = () => {
    if (manager_1.configManager.get("tableFormatter.enabled")) {
        d0._disposables.push(vscode.languages.registerDocumentFormattingEditProvider(generic_1.Document_Selector_Markdown, new MarkdownDocumentFormatter()));
    }
    else {
        d0.dispose();
    }
...
class MarkdownDocumentFormatter {
    provideDocumentFormattingEdits(document, options, token) {
        ...
        const edits = tables.map((target) => new vscode.TextEdit(target.range, this.formatTable(target, document, options)));
        return edits;
    }
    detectTables(document) {
        ...
        const result = Array.from(text.matchAll(tableRegex), (item) => ({
            text: item[0],
            offset: item.index,
            range: new vscode.Range(document.positionAt(item.index), document.positionAt(item.index + item[0].length)),
        }));
    ...
    getTableIndentation(text, options) {
        let doNormalize = manager_1.configManager.get("tableFormatter.normalizeIndentation");
    ...
    formatTable(target, doc, options) {
        ...
        const delimiterRowNoPadding = manager_1.configManager.get('tableFormatter.delimiterRowNoPadding');
        const indentation = this.getTableIndentation(text, options);
        const rowsNoIndentPattern = new RegExp(/^\s*(\S.*)$/gum);
        const rows = Array.from(text.matchAll(rowsNoIndentPattern), (match) => match[1].trim());
