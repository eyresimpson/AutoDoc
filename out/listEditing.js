...
function onEnterKey(modifiers) {
    ...
    if (/^> /.test(textBeforeCursor)) {
        ...
        return editor.edit(editBuilder => {
            if (isEmptyArrowLine) {
                const startPos = new vscode_1.Position(cursorPos.line, line.text.trim().length);
                editBuilder.delete(new vscode_1.Range(startPos, line.range.end));
        ...
        // Unordered list
        return editor.edit(editBuilder => {
            if (match3 && // If it is a task list item and
                ...
                // - [ ] 
                // - [x] |item1
                editBuilder.replace(new vscode_1.Range(cursorPos.line, match2.length + 1, cursorPos.line, match2.length + 2), " ");
                editBuilder.insert(lineBreakPos, `\n${match1}`);
            }
            else {
                // Insert "- [ ]"
...
function onBackspaceKey() {
    ...
    if (!editor.selection.isEmpty) {
        ...
        return deleteRange(editor, new vscode_1.Range(cursor.with({ character: textBeforeCursor.length - 4 }), cursor)).then(() => fixMarker(editor));
    }
    else {
 ...
 */
function findNextMarkerLineNumber(editor, line = editor.selection.start.line) {
    ...
    if ((matches = /^(\s*)([0-9]+)([.)])( +)/.exec(currentLineText)) !== null) { // ordered list
        ...
        editBuilder => {
            ...
            while (editor.document.lineCount > nextLine) {
                ...
                else if (editor.document.lineAt(nextLine - 1).isEmptyOrWhitespace // This line is a block
